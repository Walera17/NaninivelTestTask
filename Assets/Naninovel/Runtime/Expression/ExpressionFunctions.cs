...
namespace Naninovel
{
    ...
    public static class ExpressionFunctions
    {
        ...
        [Doc(
            "Return a float number in 0.0 to 1.0 range, representing how many unique commands were ever executed compared to the total number of commands in all the available naninovel scripts. 1.0 means the player had `read through` or `seen` all the available game content. Make sure to enable `Count Total Commands` in the script configuration menu before using this function.",
            "CalculateProgress()")]
        public static float CalculateProgress()
        {
            ...
            {
                Engine.Warn(
                    "`CalculateProgress` script expression function were used, while to total number of script commands is zero. You've most likely disabled `UpdateActionCountOnInit` in the script player configuration menu or didn't add any naninovel scripts to the project resources.");
                return 0;
            }
        ...
        [Doc("Checks whether an unlockable item with the provided ID is currently unlocked.",
            "IsUnlocked(\"Tips/MyTip\")")]
        public static bool IsUnlocked(string unlockableId) =>
            Engine.GetService<IUnlockableManager>()?.ItemUnlocked(unlockableId) ?? false;
